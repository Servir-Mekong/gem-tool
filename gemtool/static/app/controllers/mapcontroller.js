...
.controller('mapcontroller' ,function ($scope, $timeout, APIService) {
	...
	$scope.genMapLegend = function(data_unit) {
		...
		legend_label = [
			'< '+ parseFloat(CLASS_1.toString()),
			parseFloat(CLASS_1.toString()) + ' - '+ parseFloat(CLASS_2.toString()),
			parseFloat(CLASS_2.toString()) + ' - '+ parseFloat(CLASS_3.toString()),
			parseFloat(CLASS_3.toString()) + ' - '+ parseFloat(CLASS_4.toString()),
			'> ' + parseFloat(CLASS_4.toString()),
			'No Data',
		];
	...
	$scope.showChartDesc = function() {
		...
		var dimension_desc = {
			...
			'Myanmar' : {
				...
				"Labor Force": ["This dimension is measured by employment opportunities open to women and men in labour market, using data on the percentage of women/men who are employed, or actively seeking employment (labour fource participation rate, LFPR). ",
				"How to read Labor Index (LI) value: The closer LI to 0, the less employment opportunities for women in labour market. It is important to note that LFPR data does not reflect the quality of employment such as working conditions, fair and decent wage, social protections to workers. For example, even though women have increasingly engaged in paid employment, they might be crowed in low-skilled and precarious jobs."],
				"Violence": ["",
				""],
		...
		if(dataType === 'gii'){
			...
			selected_data= selected_data.split(":")[1];
			selected_data = selected_data.trim();
	...
	$scope.getGIISpiderChartData = function () {
		...
		APIService.getDimensionData(params)
		.then(function (result){
			var dimensions = ['Empowerment', 'Labor Force', 'Reproductive health', 'Violence'];
			...
			var chart_categories = ['Empowerment', 'Labor Force', 'Reproductive health', 'Violence'];
